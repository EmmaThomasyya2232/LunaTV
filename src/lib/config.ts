...
export function configSelfCheck(adminConfig: AdminConfig): AdminConfig {
  ...
  if (!adminConfig.HomeAds || !Array.isArray(adminConfig.HomeAds.items)) {
    adminConfig.HomeAds = { enabled: false, items: [] };
  }
  if (!adminConfig.AIRecommend) {
    adminConfig.AIRecommend = {
      enabled: false,
      baseUrl: process.env.AI_BASE_URL || '',
      model: process.env.AI_MODEL || 'gpt-5-mini',
      systemPrompt:
  ...
  adminConfig.HomeAds.enabled = Boolean(adminConfig.HomeAds.enabled);
  adminConfig.AIRecommend.enabled = Boolean(adminConfig.AIRecommend.enabled);
